...
contains
  ...
  subroutine perturb_3d(x, y, z, p0_init, s0_init, dens_pert, rhoh_pert, &
    ...
    x0 = HALF * (prob_lo(1) + prob_hi(1))
    y0 = HALF * (prob_lo(2) + prob_hi(2))
    z0 = HALF * (prob_lo(3) + prob_hi(3))

    r0 = sqrt( (x-x0)**2 + (y-y0)**2 + (z-z0)**2 ) / (0.075d0 * (prob_hi(1) - prob_lo(1)))
    ...
    eos_state%p     = p0_init
    eos_state%rho   = s0_init(rho_comp)
    if (s0_init(rho_comp) .eq. 0.d0) then
        eos_state%xn(:) = ZERO
    else
        eos_state%xn(:) = s0_init(spec_comp:spec_comp+nspec-1)/s0_init(rho_comp)
    endif
