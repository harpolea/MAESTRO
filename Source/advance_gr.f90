...
contains
  ...
  subroutine advance_timestep_gr(init_mode,mla,uold,sold,unew,snew, &
    ...
    use enforce_HSE_module          , only : enforce_TOV
    ...
    real(dp_t)    ,  intent(inout) ::  D0_old(:,0:)
    real(dp_t)    ,  intent(inout) :: Dh0_old(:,0:)
    real(dp_t)    ,  intent(inout) ::  D0_new(:,0:)
    real(dp_t)    ,  intent(inout) :: Dh0_new(:,0:)
    ...
    allocate( D0_predicted_edge(nlevs_radial,0:nr_fine))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ! 2C
    !!!! w0_force is the beta0/rho0 d(pi0/beta0)/dr term !!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    call make_macrhs(macrhs,D0_old,Source_nph,delta_gamma1_term,Sbar,div_coeff_old,dx, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (evolve_base_state) then
       call advect_base_dens(w0,D0_old,D0_new,D0_predicted_edge,dt)
    ...
    if (evolve_base_state) then
       if (use_etarho) then
          ...
          if (spherical .eq. 0) then
             ...
             call make_etarho_spherical(s1,s2,umac,w0mac,D0_old,D0_new,dx,normal, &
    ...
    if (evolve_base_state) then
       ...
       ! compute Dh0_old by "averaging"
       call average(mla,s1,Dh0_old,dx,rhoh_comp)

       call advect_base_enthalpy(w0,D0_old,Dh0_old,Dh0_new,D0_predicted_edge,psi,dt)
    else
       Dh0_new = Dh0_old
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    call make_macrhs(macrhs,D0_new,Source_nph,delta_gamma1_term,Sbar,div_coeff_nph,dx, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (evolve_base_state) then
       call advect_base_enthalpy(w0,D0_old,Dh0_old,Dh0_new, &
                                 D0_predicted_edge,psi,dt)
    else
       Dh0_new = Dh0_old
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (init_mode) then
       ...
       if (dpdt_factor .gt. ZERO) then
          ...
          call correct_hgrhs(the_bc_tower,mla,D0_new,hgrhs,div_coeff_nph,dx,dt, &
