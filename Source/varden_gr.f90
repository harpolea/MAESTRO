...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if (do_sponge) then
     if (spherical .eq. 0) then
        call init_sponge(D0_old(1,:),dx(nlevs,:),prob_lo(dm))
     else
        call init_sponge(D0_old(1,:),dx(nlevs,:),ZERO)
     end if
  end if
...
111 format(78('-'))
     ...
     if ( plot_int > 0 ) then
        ...
        do ip = 1, 2
           ...
           call write_base_state(restart, plot_file_name, &
                                 D0_old, Dh0_old, p0_old, gamma1bar, &
  ...
  if (restart < 0) then
     ...
     if (use_particles) then
        call init_particles(particles,sold,D0_old,Dh0_old,p0_old,tempbar, &
                            mla,dx,1)
...
1107    format(' ')
        ...
        if (chk_int > 0 .or. dump_checkpoint) then
           if (mod(istep,chk_int) .eq. 0 .or. dump_checkpoint) then
              ...
              call write_base_state(istep, check_file_name, &
                                    D0_new, Dh0_new, p0_new, gamma1bar(:,:), &
                                    w0, etarho_ec, etarho_cc, &
                                    div_coeff_old, psi, tempbar, tempbar_init, prob_lo(dm))
