module make_grav_module
  ...
  public :: make_grav_cell, make_dpdr_cell, make_grav_edge, make dpdr_edge, make_grav_edge_uniform
  ...
  subroutine make_dpdr_edge(dpdr_edge, Dh0, p0, u0, g, Rr, c)
    ...
    if (spherical .eq. 0) then
       ...
       if (do_planar_invsq_grav)  then
          ...
          do n=1,nlevs_radial
             do r = 1, nr(n)-1
                dpdr_edge(n,r) = -(c**2 * Dh0_edge(n,r) + p0_edge(n,r) * u0_edge(n,r)) * g / ((c**2 * (R + 2.0d0 * r_edge_loc(n,r)) - 2.0d0 * g) * u0_edge(n,r))
             enddo
          enddo
