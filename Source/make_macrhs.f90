...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine make_macrhs(macrhs,D0,Source,delta_gamma1_term,Sbar,div_coeff, &
                         dx,gamma1bar,p0,delta_p_term,dt,delta_chi, &
                         is_predictor)
       ...
       do i = 1, nfabs(Source(n))
          ...
          case (3)
             if (spherical .eq. 1) then
                call make_macrhs_3d_sphr(lo,hi,D0(1,:),mp(:,:,:,1),ng_rh,sp(:,:,:,1), &
  ...
  subroutine make_macrhs_1d(n,lo,hi,rhs,ng_rh,Source,ng_sr,delta_gamma1_term,ng_dg, &
    ...
    if (dpdt_factor .gt. 0.0d0) then
       ...
       ! Volume discrepancy term - pretty sure this is unchanged.
       do i = lo(1),hi(1)
  ...
  subroutine make_macrhs_3d_sphr(lo,hi,D0,rhs,ng_rh,Source,ng_sr,delta_gamma1_term,ng_dg, &
    ...
    if (dpdt_factor .ge. 0.0d0) then
       ...
       do k = lo(3),hi(3)
          do j = lo(2),hi(2)
             do i = lo(1),hi(1)
                if (D0_cart(i,j,k,1) .gt. base_cutoff_density) then
                   delta_chi(i,j,k) = delta_chi(i,j,k) + dpdt_factor * delta_p_term(i,j,k) / &
       ...
       deallocate(gamma1bar_cart,p0_cart,D0_cart)
