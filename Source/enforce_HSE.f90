module enforce_HSE_module
  ...
  public :: enforce_HSE, enforce_TOV
...
contains
  ...
  subroutine enforce_TOV(Dh0, p0, u0, g, Rr, c)
    ...
    integer         :: n,l,i,r
    real(kind=dp_t) :: temp,offset
    real(kind=dp_t) :: dpdr_edge(nlevs_radial,0:nr_fine-1)
    real(kind=dp_t) :: dpdr_cell(nlevs_radial,0:nr_fine-1)
    real(kind=dp_t) ::    p0_old(nlevs_radial,0:nr_fine-1)

    offset = 0.d0

    call make_dpdr_edge(dpdr_edge, Dh0, p0, u0, g, Rr, c)
    call make_dpdr_cell(dpdr_cell, Dh0, p0, u0, g, Rr, c)
    ...
    ! entire pressure so we have effectively integrated from the "top"
    do r=1,min(r_end_coord(1,1),base_cutoff_density_coord(1))
       p0(1,r) = p0(1,r-1) + dr(1) * dpdr_edge(1,r)
    end do
    do r=base_cutoff_density_coord(1)+1,r_end_coord(1,1)
       p0(1,r) = p0(1,r-1)
    end do
       ...
       do i=1,numdisjointchunks(n)
          ...
          if (r_start_coord(n,i) .eq. 0) then
             ...
             if (do_planar_invsq_grav .OR. do_2d_planar_octant .eq. 1) then
                ...
                p0(n,r_start_coord(n,i)) = p0(n-1,r_start_coord(n,i)/2-1) &
                     + (dr(n)/2.d0)* &
                      (dpdr_edge(n,r_start_coord(n,i)) + &
                       dpdr_cell(n-1,r_start_coord(n,i)/2-1))  &
                     + (dr(n)/4.d0)* &
                      (dpdr_edge(n,r_start_coord(n,i)) + &
                       dpdr_cell(n,r_start_coord(n,i)))
             else
                ! assuming constant g here
                p0(n,r_start_coord(n,i)) = p0(n-1,r_start_coord(n,i)/2-1) &
                     + (3.d0*dpdr_cell(n,i)*dr(n)/2.d0)
             endif
          else
             ! copy pressure from below
             p0(n,r_start_coord(n,i)) = p0(n-1,r_start_coord(n,i)/2-1)
          ...
          ! integrate upwards as normal
          do r=r_start_coord(n,i)+1,min(r_end_coord(n,i),base_cutoff_density_coord(n))
             p0(n,r) = p0(n,r-1) + dr(n)*dpdr_edge(n,r)
          end do
          do r=base_cutoff_density_coord(n)+1,r_end_coord(n,i)
             p0(n,r) = p0(n,r-1)
          end do
          ...
          if (r_end_coord(n,i) .eq. nr(n)-1) then
             ...
             if (do_planar_invsq_grav .OR. do_2d_planar_octant .eq. 1) then
                ...
                temp = p0(n,r_end_coord(n,i)) &
                     + (dr(n)/2.d0)* &
                      (dpdr_edge(n-1,(r_end_coord(n,i)+1)/2) + &
                       dpdr_cell(n-1,(r_end_coord(n,i)+1)/2)) &
                     + (dr(n)/4.d0)* &
                      (dpdr_cell(n,r_end_coord(n,i)) + &
                       dpdr_edge(n-1,(r_end_coord(n,i)+1)/2))
                offset = p0(n-1,(r_end_coord(n,i)+1)/2) - temp
             else
                ! assuming constant g here
                temp = p0(n,r_end_coord(n,i)) + (3.d0*dpdr_cell(n,i)*dr(n)/2.d0)
                offset = p0(n-1,(r_end_coord(n,i)+1)/2) - temp
             endif
          else
             ! copy pressure from below
