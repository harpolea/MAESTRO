...
contains
  ...
  subroutine initialize_from_restart(mla,restart,dt,pmask,dx,uold,sold,gpi,pi, &
    ...
    use metric_module, only: prim_to_cons_1d
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine initialize_with_fixed_grids(mla,dt,pmask,dx,uold,sold,gpi,pi, &
    ...
    allocate(alpha(nlevs),beta(nlevs),gam(nlevs),u0(nlevs),W(nlevs))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine initialize_with_adaptive_grids(mla,dt,pmask,dx,uold,sold,gpi,pi, &
    ...
    do n = 1,nlevs
       ...
       call multifab_build(          gam(n), mla%la(n),    dm, ng_s)
       call multifab_build(           u0(n), mla%la(n),     1, ng_s)
       call multifab_build(            W(n), mla%la(n),     1, ng_s)
  ...
  subroutine initialize_1d_arrays(num_levs,div_coeff_old,div_coeff_new,gamma1bar, &
    ...
    if (spherical .eq. 0) then
       ...
       allocate(sprim0_init       (num_levs,0:nr_fine-1,nscal))
       ...
       allocate(u0_1d         (num_levs,0:nr_fine-1))
       ...
       allocate(sprim0_init       (1,0:nr_fine-1,nscal))
